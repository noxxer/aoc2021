...
class Cube:
    def __init__(self, x_d: str, y_d: str, z_d: str):
        self.x_b, self.x_e = x_d.split('..')
        self.y_b, self.y_e = y_d.split('..')
        self.z_b, self.z_e = z_d.split('..')

    def intersection(self, c: Cube):
        dice_sum = 0
        for i in range(n):
            dice_sum += self.num
            self.num += 1
